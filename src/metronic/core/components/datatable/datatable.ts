...
export class KTDataTable<T extends KTDataTableDataInterface> extends KTComponent implements KTDataTableInterface {
	...
	private _initDefaultConfig(config?: KTDataTableConfigInterface): KTDataTableConfigInterface {
		return {
			...
			/**
			 * Info text when some error happens fetching the data
			 */
			...
			loading: {
				...
				template: `
					<div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
						<div class="flex items-center gap-2 px-4 py-2 font-semibold leading-none text-2sm border border-gray-200 shadow-default rounded-md text-gray-500 bg-light">
	...
	private async _fetchDataFromServer(): Promise<void> {
		...
		try {
			...
			this._fireEvent('fetchError');
			this._dispatchEvent('fetchError', { error });
			this._tbodyElement.innerHTML = '';
			this._noticeOnTable(this._config.errorMessage || 'Error fetching data: ' + String(error));
			return;
		}
	...
	private _noticeOnTable(message: string = ''): void {
		this._tbodyElement = this._tableElement.tBodies[0];
